...
import { UppySettingsMessage } from "#/app/messages/settings.message.js";
...
export class SettingsService {
  ...
  private async toggleUseForceOnly(interaction: ButtonInteraction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const existed = await this.getOrCreateSettings(interaction.guildId!);
  ...
  private setupChannelManagementCollector(message: Message) {
    ...
    collector.on("collect", (interaction) => {
      if (interaction.isStringSelectMenu()) {
        ...
        this.handleChannelFieldSelection(interaction, selectedField!);
  ...
  private async handleChannelSelection(
    ...
    await UppySettingsModel.updateOne(
      { guildId: interaction.guildId },
      { [field]: interaction.values[0] ?? null },
    );
  ...
  private async handleRoleFieldSelection(
    ...
    if (currentRoles && currentRoles?.length > 0) {
      roleSelector.setDefaultRoles(
        this.normalizeSnowflakes(interaction.guild!, currentRoles, "role"),
      );
    }
  ...
  private async buildAwardManagementMessage(interaction: ButtonInteraction) {
    ...
    const config = {
      ...
      disboard: entries.find(
        (e) => e.type === MonitoringType.DisboardMonitoring,
      ) ?? {
        default: PointsRate[MonitoringType.DisboardMonitoring],
        bonus: PointsRate.night,
      },
    };
    ...
    const embed = new EmbedBuilder()
      .setFields(
        ...
        {
          name: "> Disboard monitoring",
          value: [
            `Стандартно: ${config.disboard?.default}`,
            `Бонус: ${config?.disboard?.bonus}`,
          ].join("\n"),
        },
      )
      .setDefaults(interaction.user);
  ...
  private normalizeSnowflakes(
    guild: Guild,
    snowflakes: string | string[],
    type: "channel" | "role",
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ): any[] {
    function getSnowflakeEntity(s: Snowflake) {
      if (type === "role") {
        return guild.roles.cache.get(s);
      }
      if (type === "channel") {
        return guild.channels.cache.get(s);
      }
    }
    if (Array.isArray(snowflakes)) {
      return snowflakes.filter(getSnowflakeEntity);
    }
    const entity = getSnowflakeEntity(snowflakes);
